...
curl -s -XGET $ELASTICSEARCH/${INDEXNAME}2/monograph/_search -d '{"query":{"match_all":{}},"fields":["/@id"],"size":"50000"}'|grep -o "$INDEXNAME:[^\"]*" >$REGAL_LOGS/monographPids.txt
...
cat $REGAL_LOGS/monographPids.txt | parallel --jobs 2 ./lobidifyPid.sh {} $BACKEND >$REGAL_LOGS/lobidify-monographs-`date +"%Y%m%d"`.log 2>&1
